...
function KelolaPeminjamanAlat() {
  ...
  const fetchPeminjaman = async () => {
    try {
      setLoading(true)
      const data = await apiRequest('/peminjaman-alat')
  ...
  const handleUbahStatus = async (id, status) => {
    try {
      ...
      await apiRequest(`/peminjaman-alat/${id}/status`, {
        method: 'PUT',
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 px-6 py-10">
        <p>Memuat data peminjaman alat...</p>
      </div>
    )
  }
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Lihat pengajuan peminjaman alat dan ubah status peminjaman.
            </p>
          </div>
              ...
              <tbody>
                {peminjamanList.map((item) => (
                  <tr key={item.id} className="border-b align-top">
                    ...
                    <td className="px-4 py-4 text-slate-700">
                      <p>{formatTanggal(item.tanggal_pinjam)}</p>
                      <p className="text-xs text-slate-500">
                        s.d. {formatTanggal(item.tanggal_kembali)}
                      </p>
                    </td>
...
export default KelolaPeminjamanAlat